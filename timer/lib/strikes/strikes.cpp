...
static uint8_t currentStrikes = 0;

// Segment bitmaps for limited custom font
uint16_t getCustomChar(char c) {
  switch (c) {
    case 'X': return 0x280A; // Segments 1, 3, 11, 13
    case ' ': return 0x0000;
    default:  return 0x0000; // fallback (blank)
  }
}
...
void initStrikeDisplay() {
  Wire.beginTransmission(HT16K33_ALPHA_ADDRESS);
  Wire.write(0x21); Wire.endTransmission(); // Oscillator on
  Wire.beginTransmission(HT16K33_ALPHA_ADDRESS);
  Wire.write(0x81); Wire.endTransmission(); // Display on
  Wire.beginTransmission(HT16K33_ALPHA_ADDRESS);
  Wire.write(0xE0 | 0x0F); Wire.endTransmission(); // Brightness max
...
void updateAlphaDisplay(const char* txt) {
  ...
  if (len == 1) {
    // Single character → left digit only
...
void setStrikes(uint8_t strikes) {
  currentStrikes = strikes > 2 ? 2 : strikes;
...
void updateStrikeCount(uint8_t strikes) {
  ...
  // Strike == 2 → flashing effect at 4 Hz
