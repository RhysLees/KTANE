...
# macOS/Linux equivalent of build_debug.ps1
...
echo -e "${CYAN}🔧 KTANE Debug Module Build Script${NC}"
echo -e "${CYAN}===================================${NC}"

echo -e "${CYAN}🔍 Auto-detecting debug/test module directories...${NC}"
modules=()
shopt -s nullglob
for d in */; do
  [[ -d "$d" ]] || continue
  ...
  modules+=("$name")
  echo -e "${GRAY}  • Found debug module: ${name}${NC}"
done
shopt -u nullglob

if ((${#modules[@]} == 0)); then
  echo -e "${RED}❌ No debug/test modules found! Make sure your module directories contain platformio.ini files and match can_debug* or can_test* pattern.${NC}" >&2
  exit 1
fi

IFS=', '
echo -e "${GREEN}📋 Detected ${#modules[@]} debug/test modules: ${modules[*]}${NC}"
unset IFS
...
if ((successCount > 0)); then
  echo ""
  echo -e "${GREEN}📁 Debug/Test firmware files available in '${firmwareDir}' directory:${NC}"
  shopt -s nullglob
...
echo ""
if ((failCount == 0)); then
  echo -e "${GREEN}🎉 All debug/test modules built successfully!${NC}"
else
  echo -e "${YELLOW}⚠️  Some debug/test modules failed to build. Check the output above for details.${NC}"
fi
